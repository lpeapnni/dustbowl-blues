...
/datum/computer_file/program/camera_monitor/hacked/process_tick()
	...
	if(HNM && HNM.current_network && (HNM.current_network in station_networks) && prob((SKILL_LEVEL_MAX - operator_skill) * 0.05))
...
/datum/computer_file/program/camera_monitor/hacked/nano_ui_interact(mob/user)
	operator_skill = get_operator_skill(user, SKILL_SCI)
