...
/obj/machinery/chem_master/attackby(var/obj/item/B as obj, var/mob/user as mob)
	...
	if(!user.stats?.getPerk(PERK_NERD) && !user.stats?.getPerk(PERK_MEDICAL_EXPERT) && !usr.stat_check(STAT_BIO, SKILL_LEVEL_EXPERT) && !simple_machinery && !usr.stat_check(STAT_COG, 30)) //Are we missing the perk AND to low on bio? Needs 15 bio so 30 to bypass
