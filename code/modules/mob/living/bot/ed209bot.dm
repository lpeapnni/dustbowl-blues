...
/obj/item/secbot_assembly/ed209_assembly/attackby(obj/item/I, mob/user)
	...
	switch(tool_type)
		...
		if(QUALITY_WELDING)
			if(build_step == 3)
				if(I.use_tool(user, src, WORKTIME_NORMAL, tool_type, FAILCHANCE_NORMAL, required_stat = SKILL_SUR))
		...
		if(QUALITY_SCREW_DRIVING)
			if(build_step == 8)
				if(I.use_tool(user, src, WORKTIME_NEAR_INSTANT, tool_type, FAILCHANCE_NORMAL, required_stat = SKILL_SUR))
