...
/obj/item/tool/saw/ripper
	name = "Ripper"
	desc = "A Ripper vibroblade. Powered by a medium energy cell, the chainblade rips and tears into its target."
	icon = 'icons/fallout/objects/melee/melee.dmi'
	icon_state = "ripper"
	item_state = "chainswordoff"
	switched_on_item_state = "chainswordon"
	switched_on_icon_state = "ripper_on"
	wielded_icon = "chainswordon"
	hitsound = WORKSOUND_CHAINSAW
	worksound = WORKSOUND_CHAINSAW
	force = WEAPON_FORCE_WEAK
	switched_on_forcemult = 4 //28 total
...
/obj/item/tool/saw/ripper/turn_on(mob/user)
	if (cell && cell.charge >= 1)
		item_state = "[initial(item_state)]_on"
		to_chat(user, SPAN_NOTICE("You rev up the [src]."))
		playsound(loc, 'sound/items/chainsaw_on.ogg', 40)
...
/obj/item/tool/saw/ripper/turn_off(mob/user)
	playsound(loc, 'sound/items/chainsaw_off.ogg', 80)
	to_chat(user, SPAN_NOTICE("You turn the [src] off."))
	..()
