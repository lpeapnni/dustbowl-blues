...
/obj/structure/flora/grass/attackby(obj/item/I, mob/user)
	...
	if(tool_type==QUALITY_CUTTING)
		...
		if(I.use_tool(user, src, WORKTIME_NEAR_INSTANT, tool_type, FAILCHANCE_ZERO, required_stat = SKILL_SUR))
...
/obj/structure/flora/small/attackby(obj/item/I, mob/user)
	...
	if(tool_type==QUALITY_HAMMERING)
		...
		if(I.use_tool(user, src, WORKTIME_NORMAL, tool_type, FAILCHANCE_ZERO, required_stat = SKILL_SUR))
